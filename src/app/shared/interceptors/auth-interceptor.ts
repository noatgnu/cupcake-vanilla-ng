...
export const authInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (req.url.includes('/auth/') || req.url.includes('/site-config/public/')) {
    return next(req);
  }
...
function handle401Error(request: HttpRequest<any>, next: HttpHandlerFn, http: HttpClient, router: Router): Observable<HttpEvent<unknown>> {
  if (!isRefreshing) {
    isRefreshing = true;
    ...
    if (refreshToken) {
      return http.post<{access: string}>(`${environment.apiUrl}/auth/token/refresh/`, {
